...
export default function Area({ multiplier }: AreaProps) {
  if (multiplier === 0) {
    return <div></div>;
  }
  return (
    <div
      className={cn(
        "left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-300/50 text-transparent",
        ...
        multiplier === 10 && "h-[300px] w-[300px]", // 30px * 10
        multiplier === 14 && "h-[420px] w-[420px]", // 30px * 14
        multiplier === 15 && "h-[450px] w-[450px]", // 30px * 15
        multiplier === 21 && "h-[630px] w-[630px]", // 30px * 21
      )}
    ></div>
  );
}
