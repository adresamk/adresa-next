...
export default function SharedAdditionalFeatures({
  ...
  const sharedFeatures = allCategoryFeatures;

  const sharedFeaturesCategories: Record<FeatureCategory, Feature[]> =
    sharedFeatures.reduce(
  ...
  return (
    <>
      ...
      {/* Feature block */}
      {Object.entries(sharedFeaturesCategories).map(([category, features]) => (
        <div key={category}>
          <h3 className="my-2.5 text-lg">
